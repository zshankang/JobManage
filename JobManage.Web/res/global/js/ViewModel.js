...
function ViewModel(options) {
    ...
    var defaults = {
        ...
        showGridList: function (options) {
            ...
            var grid = job.grid.render(options.gridListControlID, options);
        ...
        formLoad: function (options) {
            ...
            var defaults = {
                ...
                success: function (result) {
                    //将数据反序列化绑定到表单上
                    job.form.deserialize(options.formControlID, result.Data);
                    //查看详情页面需要设置表单控件只读
        ...
        submitForm: function (url) {
            var formID = "form1";
            if (!job.form.valid(formID)) {
                return false;
            }
            job.form.submit({
                url: url,
                params: job.form.serialize(formID),
                success: function (result) {
                    job.common.currentWindow().$("#gridList").resetSelection();
                    job.common.currentWindow().$("#gridList").trigger("reloadGrid");
                }
            });
        },
        //点击Add按钮弹出“添加数据”对话框
