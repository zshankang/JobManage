...
(function ($) {
     ...
     */
    job.common.loading = function (isShow, text) {
     ...
     * @return {Object}  返回键值对
     */
    job.common.getQueryStringArgs = function () {
        //取得查询字符串并去掉问号
     ...
     */
    job.common.request = function (name) {
        var args = job.common.getQueryStringArgs();
        var result = "";
        if (args) {
    ...
    * @return {Object}  返回键值对
    */
    job.common.getRequestData = function () {
        var requestData = job.common.getQueryStringArgs();
        //id参数
     ...
     * @return {Object}  
     */
    job.common.currentWindow = function () {
     ...
     */
    job.common.download = function (url, data, method) {
        if (url && data) {
     ...
     */
    job.modal.open = function (options) {
        var defaults = {
            id: null,
            title: '系统窗口',
            width: "100px",
     ...
     * @param  {Number} type  提示类型 0 成功 1 错误
     */
    job.modal.alert = function (content, type) {
     ...
     * @param  {Number} type  提示类型 0 成功 1 错误
     */
    job.modal.msg = function (content, type) {
        if (type != undefined) {
            var icon = "fa-check-circle";
            var cssType = 'success';
            switch (type) {
     ...
     */
    job.modal.confirm = function (content, callBack) {
     ...
     * @param  {Object} options  选项
     */
    job.form.load = function (options) {
        var defaults = {
            url: "",
            params: [],
            loading: "数据加载中，请稍候…",
        ...
        window.setTimeout(function () {
            $.ajax({
                ...
                data: options.params,
                beforeSend: function () {
                    job.common.loading(true, options.loading);
                },
                success: function (data) {
                    if (data.Status == 0) {
                        ...
                        job.modal.alert(data.Message, data.Status);
                    }
                }
            });
        }, 0);
     ...
     * @param  {Object} options  选项
     */
    job.form.submit = function (options) {
        var defaults = {
            url: "",
            params: [],
            loading: "正在提交数据...",
        ...
        window.setTimeout(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.Status == 0) {
                        options.success(data);
                        job.modal.msg(data.Message, data.Status);
     ...
     * @param  {Object} options  选项
     */
    job.form.delete = function (options) {
        var defaults = {
        ...
        //弹出是否确认删除
        job.modal.confirm(options.prompt, function (r) {
            if (r) {
                //job.common.loading(true, options.loading);
                //显示正在加载遮罩层
                window.setTimeout(function () {
     ...
     * @param  {Object} options    选项
     */
    job.form.bindSelect = function ($elements, options) {
        var defaults = {
            id: "ID",
            text: "Name",
            search: false,
     ...
     * @return {Boolean} 键值对
     */
    job.form.serialize = function (id) {
        var element = $("#" + id);
        var postdata = {};
     ...
     * @param  {Object} data 数据
     */
    job.form.deserialize = function (id, data) {
        var element = $("#" + id.toLowerCase());
        if (!data) {
            return;
        }
        for (var key in data) {
            var $id = element.find('#' + key.toLowerCase());
            if (!$id) {
                return;
            }
     ...
     * @param {any} strTime
     */
    job.form.HandelTime = function (strTime) {
     ...
     * @param  {Object} options 选项
     */
    job.grid.render = function (id, options) {
        var defaults = {
            datatype: "json",
            autowidth: true,
            rownumbers: true,
            ...
            loadError: function (xhr, textStatus, errorThrown) {
                job.common.loading(false);
                //登陆超时
                //if (xhr.status == 401) {
                //    job.modal.alert("登陆超时，请重新登陆！", 0);
                //    setTimeout(function () {
                //        top.location.href = "/Login/Index";
                //    }, 1000);
                //} else if (xhr.status == 403) {
                //    job.modal.alert("无权限访问！", 0);
                //}
                //else {
                //    job.modal.msg(errorThrown, 0);
                //}
            },
     ...
     * @param  {String} id      控件id
     */
    job.grid.getRowValue = function (id) {
        var $grid = $("#" + id);
    ...
    */
     ...
     * @return {Object}
     */
    job.auth.current = null;

    /**
     * 允许的按钮
     * @param  {Object}  options 选项 
     */
    job.auth.grantedButtons = function (options) {
        var defaults = {
        ...
        if (!job.auth.current) {
            //获取当前页面所拥有的权限对象
            job.auth.current = getMenuPermission();
        }
        var current = job.auth.current;
        //授权的按钮
     ...
     * @param  {Object}  options 选项 
     */
    job.auth.fieldIsGranted = function (fieldName) {
        //权限对象是否缓存，没有则缓存上
        if (!job.auth.current) {
            //获取当前页面所拥有的权限对象
            job.auth.current = getMenuPermission();
...
$(function () {
    ...
    $.ajaxSetup({
        ...
        error: function (xhr, textStatus, errorThrown) {
            ...
            //    }, 1000);
            //} else if (xhr.status == 403) {
            //    job.modal.alert("无权限访问" + xhr.statusText + "！", 0);
            //}
            //else {
            //    job.modal.msg(errorThrown, 0);
            //}
    ...
    job.form.bindSelect($(".select2-control"));
    //body设置主题样式
